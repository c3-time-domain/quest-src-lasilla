...
// skip weather check
    ...
    sprintf (string,"echo ERROR > %s 2>&1",weather_file);
    ...
    sprintf(string,"echo s | %s > %s 2>&1",weather_client,weather_file);
    fprintf(stderr,"get_weather_data: executing [%s]\n",string);
    system (string);
    ...
    if(input==NULL){
      ...
      fprintf(stderr,"get_weather_data: weather file line is [%s]\n",string);
