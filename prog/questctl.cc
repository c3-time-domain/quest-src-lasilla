...
int getmjd(double *mjd);
int get_lst_mjd(double *lst, double *mjd);
...
double fake_ut_offset = 0; // offset added to actual UT for simulated obs
using namespace std;
...
//    double current_ra, current_dec;
    ...
    fprintf(stderr,"questctl: %d arguments\n",argc);

    int i;;
    for (i=1;i<=argc-1;i++){
      fprintf(stderr,"questctl: arg %d [%s]\n",i,argv[i]);
      ...
      if(strncmp(argv[i],"-h",2)==0){
       ...
       do_exit(0);
         ...
         dome_override_flag=1;
      }
      else if(strncmp(argv[i],"-t",2)==0){
         sscanf(argv[i+1],"%lf",&fake_ut_offset);
      }
    }

    fprintf(stderr,"questctl: fake_site_flag: %d  wait_for_dome: %d  dome_override_flag: %d fake_ut_offset %7.3f\n",fake_site_flag,wait_for_dome, dome_override_flag,fake_ut_offset);
    ...
    *reply=0;
    ...
    fprintf(stderr,"current ut with offset is %12.6f\n", get_date_time(&tm,fake_ut_offset));
    ...
    if(fake_site_flag){
        ...
        my_fake_stop_time=neat_gettime_utc(fake_ut_offset);
        my_fake_status=TRACKING_OFF_STATE;
        strcpy(fake_com1_status,"E");   
        record_fake_status(my_fake_dome_status);
    }
    else{
      if(dome_override_flag){
            fprintf(stderr,"%s: operations allowed with dome closed. No dome wait",argv[0]);
        fflush(stderr);
      }
      ...
      telescope = new telescope_controller(site_info.com_port(),
         site_info.mount_point_timeout(), site_info.server_name(),
         fake_ut_offset);
      if(telescope==NULL){
      ...
      if (wait_for_dome){
        while(telescope->dome_shutter_status()!=DOME_OPEN_STATUS){
          if(get_command_code()<0){
            fprintf(stderr,
              "error getting command code while waiting for dome to open\n");
            do_exit(-1);
          }
          if(sunup(&almanac)==SUNUP){
            ...
            fprintf(stderr,"%s: %9.6f Waiting for dome to open. \n",
              argv[0],neat_gettime_utc(fake_ut_offset));
            sleep(60);
          }// if(get_command_code()<0)
        } // while(telescope->dome_shutter_status()!=DOME_OPEN_STATUS)
      ...
      while(t==NULL){ 
        t = new telmount(fake_site_flag,dome_override_flag,fake_ut_offset);
        if ((t == NULL) || t->fail)
        {
      ...
      */
      ...
      while(nfds==0){
        ...
        if((t2.tv_sec-t1.tv_sec)>STATUS_CHECK_INTERVAL){
           ...
           if(get_command_code()<0){
               fprintf(stderr,
                 "error getting command code while checking status\n");
               do_exit(-1);
           }
...
int get_position(double *ra, double *dec, double *ut)
{
   ...
   if(fake_site_flag){
     ...
     *ra=lst-my_fake_ha;
     my_fake_ra=*ra;
     *dec=my_fake_dec;
     my_fake_ut=neat_gettime_utc(fake_ut_offset);
     *ut=my_fake_ut;
   }
   else {
...
int process_command(char *command_str, char *reply, int socfd)
{
    ...
    if (strlen(command_str)>1) {
      ...
      if(strcmp(command,CLOSEDOME_COMMAND)==0){
          if(domectl(telescope,DOME_CLOSED_STATE)==0){
              sprintf(reply,"%s\n",COMMAND_DONE_REPLY);
          }
          else{
      ...
      else if(strcmp(command,OPENDOME_COMMAND)==0){
          if(domectl(telescope,DOME_OPEN_STATE)==0){
              sprintf(reply,"%s\n",COMMAND_DONE_REPLY);
          }
          else{
      ...
      else if(strcmp(command,GETFOCUS_COMMAND)==0){
      ...
      else if(strcmp(command,SETFOCUS_COMMAND)==0){
          sscanf(command_str,"%s %lf",dummy_string,&f_mm);
      ...
      else if(strcmp(command,LST_COMMAND)==0){
          if(getlst(&lst)==0){  
      ...
      else if(strcmp(command,DOMESTATUS_COMMAND)==0){
          dome_status=get_domestatus(telescope, dome_status_string);
          if(dome_status<0){
      ...
      else if(strcmp(command,STATUS_COMMAND)==0){
          if(get_tracking_status(&tracking_status)!=0){
      ...
      else if(strcmp(command,SLAVEDOME_COMMAND)==0){
      ...
      else if(strcmp(command,WEATHER_COMMAND)==0){
      ...
      else if(strcmp(command,FILTER_COMMAND)==0){
       ...
       "process_command: error sending reply to socket\n");
          ...
          if(t->interp(socfd,command_str)!=0){
            sleep(3);
            fprintf(stderr,"process_command: retrying failed command: %s\n",command);
            // try once more:
            if(t->interp(socfd,command)!=0) return -1;
            fprintf(stderr,"process_command: command success on second attempt:: %s\n",command);
          }
        }
      }
    }// if (strlen(command)>1) {
...
void print_help()
{
   printf("usage: questctl [-f for simulation] [-o to allow ops with dome closed] [-d to wait for dome open] [-t xx.x where xx.x is UT time offset]\n");
...
int fake_interp(char *command_str, char *reply)
{
    char string[COMMAND_LENGTH];
    double ut_time,lst,mjd;
    char command[1024];
    ut_time=neat_gettime_utc(fake_ut_offset);

    sscanf(command_str,"%s",command);

    if(strcmp(command,POINT_COMMAND)==0){
       cerr << "fake point command" << endl;
       sleep(10);
       sscanf(command_str,"%s %lf %lf",string,&my_fake_ra,&my_fake_dec);
       getlst(&lst);
    ...
    else if(strcmp(command,POSRD_COMMAND)==0){
       cerr << "fake posrd command" << endl;
       getlst(&lst);
       my_fake_ra=lst-my_fake_ha;
       sprintf(reply,"%s %9.6f %9.6f %9.6f",COMMAND_DONE_REPLY,my_fake_ra,my_fake_dec,neat_gettime_utc(fake_ut_offset));
    }
    else if(strcmp(command,TELESCOPE_STATUS_COMMAND)==0){
       cerr << "fake telescope_status command" << endl;
       get_lst_mjd(&lst,&mjd);
       my_fake_ra=lst-my_fake_ha;
       my_fake_ha=lst-my_fake_ra;

      switch(my_fake_dome_status){
      case ds_open:
      ...
      //record_fake_status(my_fake_dome_status);
      fprintf(stderr,"Telescope RA: %10.6f  ",my_fake_ra);
      fprintf(stderr,"Telescope Dec: %10.6f  ",my_fake_dec);
      fprintf(stderr,"Telescope HA: %10.6f  ",my_fake_ha);
      fprintf(stderr,"Telescope LST: %10.6f  ",lst);
      fprintf(stderr,"Telescope JD: %10.1f  ",mjd+24500000.5);
      fprintf(stderr,"Telescope UT: %10.6f  ",ut_time);
      fprintf(stderr,"Telescope Epoch: %6.1f  ",2000.0);
      fprintf(stderr,"Telescope Azimuth: %6.1f  ",-1.0);
      fprintf(stderr,"Telescope Elevation: %6.1f  ",-1.0);
      fprintf(stderr,"Telescope Secz: %6.1f  ",-1.0);
      fprintf(stderr,"Focus: %6.3f  ",0.0);
      fprintf(stderr,"Dome Position: %5.1f  ",0.0);
      fprintf(stderr,"Wobble Status: %s  ","UNKNOWN");
      fprintf(stderr,"Horizon Limit: %s  ","OUT_OF_LIMIT");
      fflush(stderr);
      sprintf(reply,COMMAND_DONE_REPLY);
    }
    else if(strcmp(command,STOW_COMMAND)==0){
    ...
    else if(strcmp(command,STOPMOUNT_COMMAND)==0){
       sleep(1);
       my_fake_status=TRACKING_OFF_STATE;
       my_fake_stop_time=neat_gettime_utc(fake_ut_offset);
       sprintf(reply,COMMAND_DONE_REPLY);
...
int getmjd(double *mjd)
{
   ...
   get_lst_mjd(&lst,mjd);
   if(verbose){
     fprintf(stderr,"getmjd: mjd is %10.7f\n",*mjd);
     fflush(stderr);
   }
   return (0);
}
...
int get_command_code()
{
  ...
  if(input==NULL){
    ...
    if(strcmp(command_string,NOCOMMAND_CODE_STRING)==0){
          command_code=NO_COMMAND_CODE;
    }
    else if (time > command_time){
       if(strcmp(command_string,STOP_CODE_STRING)==0){
          command_code=STOP_CODE;
       }
       else if (strcmp(command_string,SHUTDOWN_CODE_STRING)==0){
          command_code=SHUTDOWN_CODE;
       }
       else if (strcmp(command_string,STOW_CODE_STRING)==0){
          command_code=STOW_CODE;
       }
       else if (strcmp(command_string,CLOSE_CODE_STRING)==0){
          command_code=CLOSE_CODE;
       }
       else if (strcmp(command_string,OPEN_CODE_STRING)==0){
          command_code=OPEN_CODE;
       }
       else if (strcmp(command_string,EXIT_CODE_STRING)==0){
          command_code=EXIT_CODE;
       }
       else{
          fprintf(stderr,
